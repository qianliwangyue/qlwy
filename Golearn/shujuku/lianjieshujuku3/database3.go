...
func queryRow() {
	...
	err := db.QueryRow("select sno,sname,classname from `stuinfo2` where sno=?", 201901152).Scan(&u.sno, &u.sname, &u.classname)
	if err != nil {
		fmt.Printf("scan failed, err:%v\n", err)
		return
	}
	fmt.Printf("sno:%d sname:%s classname:%s\n", u.sno, u.sname, u.classname)
}
...
// 查询多条数据示例
	...
	func queryMultiRow() {
		rows, err := db.Query("select sno,sname from `stuinfo2` where uid = ?", 201901152)
		if err != nil {
			fmt.Printf("query failed, err:%v\n", err)
			return
		}
